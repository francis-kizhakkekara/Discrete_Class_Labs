...
// Project 2: This program finds the number of lattice paths
//            of a square grid from one corner to the opposite corner
//            Grid size >= 1
// Ex: The number of lattice paths of a 2x2 grid is 6
// An integer library called InfInt is used to store the numbers

// Library was embedded because the lab had a 1-file upload requirement
// My code starts at line 1391
...
InfInt facto(int inputNum)
{

    InfInt theFactorial = 1;
...
// This function does (2n choose n)
// The number of latice paths of a square grid is (2n choose n)
InfInt latticePaths(int gridSize)
{

    return facto(2 * gridSize) / (facto(gridSize) * facto(gridSize));
}

int main()
{

    cout << "Lattice Path (from one corner to opposite corner) Calculator" << endl;

    // Input: getting a gridsize of n for an n x n grid, n >= 1
    int theGridSize = -1;
    while (theGridSize < 1)
    {
        cout << "Enter the grid size to find the number of lattice paths. (n >= 1)" << endl;
        cin >> theGridSize;
    }

    // Output
    cout << "The number of lattice paths of a " << theGridSize << "x" << theGridSize
         << " is " << latticePaths(theGridSize) << endl;
